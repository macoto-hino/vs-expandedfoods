...
namespace ExpandedFoods
{
    public class BlockEntityExpandedOven : BlockEntityDisplay, IHeatSource
    {
        ...
        protected virtual void IncrementallyBake(float dt, int slotIndex)
        {
            ...
            float[] sats = (slot.Itemstack?.Attributes["expandedSats"] as FloatArrayAttribute)?.value;
            ...
            if (currentLevel > levelTo)
            {
                ...
                if (resultCode != null)
                {
                    ...
                    if (resultStack != null)
                    {
                        if (ings != null) resultStack.Attributes["madeWith"] = new StringArrayAttribute(ings);
                        if (sats != null) resultStack.Attributes["expandedSats"] = new FloatArrayAttribute(sats);
