...
namespace ExpandedFoods
{
    ...
    public class BlockEntityMixingBowl : BlockEntityOpenableContainer
    {
        ...
        {
            get { return Lang.Get("expandedfoods:Mixing Bowl"); }
        }
        ...
        public override void Initialize(ICoreAPI api)
        {
            ...
            if (ambientSound == null && api.Side == EnumAppSide.Client)
            {
                ...
                {
                    Location = new AssetLocation("expandedfoods:sounds/block/mixing.ogg"),
                    ShouldLoop = true,
        ...
        internal MeshData GenMesh(string type = "base")
        {
            ...
            mesher.TesselateShape(block, Api.Assets.TryGet("expandedfoods:shapes/block/" + Block.FirstCodePart() + "/" + type + ".json").ToObject<Shape>(), out mesh);
        ...
        public string GetOutputText()
        {
            CookingRecipe recipe = GetMatchingMixingRecipe(Api.World, IngredStacks);
            DoughRecipe drecipe = GetMatchingDoughRecipe(Api.World, IngredSlots);
            string locked = invLocked ? Lang.Get("expandedfoods:(Locked) ") : "";
        ...
        {
            Api.World.PlaySoundAt(new AssetLocation("expandedfoods:sounds/lock.ogg"), Pos.X, Pos.Y, Pos.Z, player);
            if (invLocked)
            {
                invLocked = false;
                return;
