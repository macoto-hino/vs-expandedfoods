...
namespace ExpandedFoods
{
    public class SaucepanInFirepitRenderer : IInFirepitRenderer
    {
        ...
        public SaucepanInFirepitRenderer(ICoreClientAPI capi, ItemStack stack, BlockPos pos, bool isInOutputSlot)
        {
            ...
            capi.Tesselator.TesselateShape(saucepanBlock, capi.Assets.TryGet("expandedfoods:shapes/block/" + saucepanBlock.FirstCodePart() + "/" + "empty.json").ToObject<Shape>(), out saucepanMesh); // Main Shape
            //potMesh.Rgba2 = null;
            saucepanRef = capi.Render.UploadMesh(saucepanMesh);

            MeshData topMesh;
            capi.Tesselator.TesselateShape(saucepanBlock, capi.Assets.TryGet("expandedfoods:shapes/block/" + saucepanBlock.FirstCodePart() + "/" + "lid-only.json").ToObject<Shape>(), out topMesh); // Lid
            //lidMesh.Rgba2 = null;
